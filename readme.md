...
#### 3.1.2 页节点插入数据 
选取具有代表性的步骤进行说明，已知节点的阶为3，故而如果插入节点的数据超过3，就需要对节点进行分裂，生成两个节点，新节点的最大值提取出来，再插入到父节点，父节点的个数也会增加，所以也需要对父节点进行分裂判断和操作，这样一直向上进行分裂判断和操作，直到根节点： 
</br>
...
**a. 插入100到98:**
如图5所示，插入100，99，98 三个数据到数据节点，3个数据可以放到未操作数据节点的阶数，故而可以放在一个叶子节点中，根节点的cell用了一对KV：V指向叶子节点， K即100(所指向叶子节点中数据最大值)。
